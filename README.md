...
- **起動オプション・Scene入力**: アプリ起動時の引数として画像ファイルを渡して起動と同時に読み込めるほか、`.scene.json` ファイルを用いることで複数の画像や配置状態を一括で初期状態として適用できます。
- **設定の永続化**: ウィンドウサイズ、表示位置、背景色、プリセットカラー、言語設定などは自動的に保存され、次回起動時に復元されます。
