...
export const DimensionLineLayer = memo(function DimensionLineLayer(
    ...
    const { dimensionLines, unitFactor, isSelected, onSelect, onUpdate, isDimensionMode } = props;

    // ラベルの
    const getLabel = (start: { x: number; y: number }, end: { x: number; y: number }) => {
    ...
    const getMidPoint = (start: { x: number; y: number }, end: { x: number; y: number }) => {
        return {
            x: (start.x + end.x) / 2,
            y: (start.y + end.y) / 2,
        };
    ...
    const handleDragEnd = useCallback(
        (id: string, type: "start" | "end") => (e: KonvaEventObject<DragEvent>) => {
                        ...
                        {selected && isDimensionMode && (
                            <>
                                <Circle
                                    ...
                                    fill="red"
                                    draggable
                                    onDragMove={handleDragMove(line.id, "start")}
